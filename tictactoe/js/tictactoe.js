...
function rollForTurn() {
    ...
    if (pOne > pTwo) {
        first = "Player 1";
        setTimeout(function() {txt1 = txt1 + "Player 1 wins, please choose a sqaure.";}, 2000);
...
// this function style the game buttons while they are disabled
...
// square has already been assigned and if it has not, record a new square with the assigned avatar
function recordMoves(square) {
    ...
    verdict = check(info,square); // call function to check if proposed sqaure is already occupied
    return verdict;
}
...
function checkForWinCon() {
    var squareArray = [];
    var target = document.getElementById('boardState');
    var info = target.innerHTML; // raw array with squares and 
    ...
    // call this following array of functinos to check for any of the possible win cons
...
function check4Tie() {
    ...
    boardState = boardState.substring(1); // removing leading comma
...
function winner(winDetected,winCon) {
    if(winDetected == "win") {
        ...
        glowBoard(winCon); // call function to make th gameboard pulse with colors
    }
}
...
// these functions will product game sounds depending on the occasion
...
// checking for wincon sqaures 147
...
function square1Animate() {
    ...
    if(activePlayer != "Game Stopped") { // if game has not yet started prevent avatar placement
        ...
        if (verdict == undefined) { // if verdict is empty than the square is unoccupied.
            ...
            // build new arrary adding the newly selected square and the assigned avatar
